...
for (i in kmeans_PCA){
  df_kmeans <- data.frame(df_kmeans, i$cluster)
  counter <- counter + 1
}

while (counter>0){
  name <- paste("Cluster", counter)
  n_total<- ncol(df_kmeans)
  
  colnames(df_kmeans)[n_total - counter + 1] <- name
  
  df_kmeans$name <- factor(df_kmeans$name)
  counter <- counter - 1
}
...
ggplot(df_kmeans, aes(EV2, EV1, color = "Cluster 2", shape = "Cluster 1")) + geom_point() + scale_color_manual(values=colores) + theme_light()
